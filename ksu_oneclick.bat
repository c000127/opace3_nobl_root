@echo off
setlocal
chcp 65001 >nul

set "DIR=%~dp0"
set "P=%DIR%prompts\"
set "KO=%DIR%android13-5.15_kernelsu.ko"
set "KSUD=%DIR%ksud-aarch64-linux-android"

type "%P%banner.txt"
echo.

echo [1/4] Checking ADB...
:check_adb
adb kill-server >nul 2>&1
adb start-server >nul 2>&1
adb get-state >nul 2>&1
if not errorlevel 1 goto check_root

type "%P%no_adb.txt"
echo.
pause >nul
goto check_adb

:check_root
set "USER_ID="
for /f "tokens=*" %%i in ('adb shell id') do set "USER_ID=%%i"
echo %USER_ID% | findstr "uid=0(root)" >nul
if not errorlevel 1 goto root_ok

type "%P%no_root.txt"
echo.
pause >nul
goto check_root

:root_ok
type "%P%root_ok.txt"
echo.
echo.

echo [2/4] Pushing files...
adb push "%KO%" /data/local/tmp/kernelsu.ko >nul 2>&1
adb push "%DIR%ksu_step2.sh" /data/local/tmp/ksu_step2.sh >nul 2>&1
adb push "%DIR%ksu_step3.sh" /data/local/tmp/ksu_step3.sh >nul 2>&1
adb push "%KSUD%" /data/local/tmp/ksud-aarch64 >nul 2>&1
adb shell "sed -i 's/\r$//' /data/local/tmp/ksu_step2.sh" >nul 2>&1
adb shell "sed -i 's/\r$//' /data/local/tmp/ksu_step3.sh" >nul 2>&1
adb shell "rm -rf /data/local/tmp/ksu_hide_module" >nul 2>&1
adb push "%DIR%ksu_hide_module" /data/local/tmp/ksu_hide_module >nul 2>&1
adb shell "find /data/local/tmp/ksu_hide_module -name '*.sh' -exec sed -i 's/\r$//' {} +" >nul 2>&1
echo [OK] Done.
echo.

echo [3/4] Loading KernelSU (ksud debug insmod)...
adb shell "sh /data/local/tmp/ksu_step2.sh" >nul 2>&1
echo [OK] Done.
echo.

type "%P%reroot.txt"
pause >nul

:check_root_phase2
echo reconnecting adb...
adb kill-server >nul 2>&1
adb start-server >nul 2>&1
set "USER_ID="
for /f "tokens=*" %%i in ('adb shell su -c id 2^>nul') do set "USER_ID=%%i"
echo %USER_ID% | findstr "uid=0(root)" >nul
if not errorlevel 1 goto root_ok_phase2

type "%P%retry_root.txt"
echo.
pause >nul
goto check_root_phase2

:root_ok_phase2
type "%P%root_reacquired.txt"
echo.
echo.

echo [4/4] Deploying environment...
adb shell "su -c sh /data/local/tmp/ksu_step3.sh" >nul 2>&1

adb shell "su -c grep -q kernelsu /proc/modules"
if errorlevel 1 goto cleanup

type "%P%success.txt"
adb shell su -c am force-stop me.weishu.kernelsu >nul 2>&1
adb shell su -c am start -n me.weishu.kernelsu/.ui.MainActivity >nul 2>&1

:cleanup
adb shell su -c rm -f /data/local/tmp/ksu_step2.sh >nul 2>&1
adb shell su -c rm -f /data/local/tmp/ksu_step3.sh >nul 2>&1
adb shell su -c rm -f /data/local/tmp/kernelsu.ko >nul 2>&1


echo.
pause
goto :eof

:fail
echo.
type "%P%abort.txt"
echo.
pause
